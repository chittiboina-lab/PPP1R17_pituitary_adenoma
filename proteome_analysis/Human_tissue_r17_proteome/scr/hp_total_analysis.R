...
lll <- read.table("data/hp_data_total.txt",header=T,row.names=1)
...
write.table(running.pvs, "total/differential_testing_results.txt", sep = "\t")

coded.keepers <- dimnames(running.pvs)[[1]][running.pvs[,5]==1]
grab.keeper.info <- running.pvs[coded.keepers,]
write.table(grab.keeper.info, "total/selected_differential.txt", sep = "\t")
...
pdf("total/post_selection_volcano_plot.pdf")
...
# cdat <- cor(cdat)
# write.table(cdat, "hp_total_cor.txt", sep = '\t')
# 
...
# 
# svg("total/post_selection_cor_heatmap.svg")
# pheatmap(cdat,annotation_colors=my_colour,annotation_col=my_sample_col,cellwidth=40,cellheight=40)
# dev.off()
# 
# temp1 <- Final.working.data[coded.keepers,]
# write.table(temp1, "hp_total_HM.txt", sep = '\t')
# cc <- colorpanel(64, "green", "red")
# heatmap.2(as.matrix(temp1), trace="none", density.info="none",col=cc,labRow="",margins=c(15,5),scale="row")
# pdf("total/post_selection_heatmap.pdf")
