...
lll <- read.table("data/mp_data_phospho.txt",header=T,row.names=1)
...
pdf('phospho/pre_normalization_boxplot.pdf')
...
pdf('phospho/post_normalization_boxplot.pdf')
...
write.table(Final.working.data,"phospho/Final_working_data_txt",sep="\t")
...
write.table(running.pvs, "phospho/R17_vs_GFP_differential_testing_results.txt", sep = "\t")

coded.keepers <- dimnames(running.pvs)[[1]][running.pvs[,5]==1]
grab.keeper.info <- running.pvs[coded.keepers,]
write.table(grab.keeper.info, "phospho/R17_vs_GFP_selected_differential.txt", sep = "\t")
...
write.table(lll[coded.keepers,], '../vis_data/mp_selected_phospho.txt', sep = '\t')
