...
lll <- read.table("data/F_data_total.txt",header=T,row.names=1)
...
pdf("total/pre_norm_box_plot.pdf")
...
pdf("total/post_norm_box_plot.pdf")
...
write.table(Final.working.data,"total/Final_working_data_total.txt",sep="\t")
...
for (i in 1:dim(Final.working.data)[1]) {
  ...
  temp7 <- temp7*(-1) # reverse difference to be Fing vs R17
...
write.csv(GFP_vs_r17, 'total/limma/GFP_vs_r17_total.csv')

GFP_vs_fing <- topTable(fit2, coef=2, adjust="BH", number = length(rownames(lll)))
write.csv(GFP_vs_fing, 'total/limma/GFP_vs_fing_total.csv')

r17_vs_fing <- topTable(fit2, coef=3, adjust="BH", number = length(rownames(lll)))
write.csv(r17_vs_fing, 'total/limma/r17_vs_fing_total.csv')

results <- decideTests(fit2)
pdf('total/limma/limma_venn.pdf')
vennDiagram(results)
dev.off()
...
write.table(running.pvs, "total/R17_vs_Fing_differential_testing_results.txt", sep = "\t")

coded.keepers <- dimnames(running.pvs)[[1]][running.pvs[,5]==1]
grab.keeper.info <- running.pvs[coded.keepers,]
write.table(grab.keeper.info, "total/R17_vs_Fing_selected_differential.txt", sep = "\t")
...
#-------------------------------------------------------------------------------
write.table(sig_raw, '../vis_data/f_total_limma_selection.txt', sep = '\t')
write.table(lll[coded.keepers,], '../vis_data/f_selected_total.txt', sep = '\t')
